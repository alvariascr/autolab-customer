-- Fix: book_service_appointment validated product price/quantity but never
-- checked or reserved inventory_items.current_stock, unlike create_cart_order
-- (202607310001/202608110002), which does both atomically. Because
-- cancel_order_and_release_resources restores stock for every 'pending'
-- order_products row on cancellation/TTL-expiration regardless of order
-- type, every cancelled appointment-with-products order was adding stock
-- back that was never actually reserved -- inflating inventory on a routine,
-- expected flow (not an edge case).
--
-- This mirrors create_cart_order's exact two-step pattern:
--  1. During validation, lock each candidate inventory_items row
--     (`for no key update`) and reject if current_stock < requested
--     quantity.
--  2. Right before inserting each order_products row, atomically decrement
--     current_stock with a re-check in the WHERE clause, raising
--     appointment_product_stock_unavailable if it can no longer be
--     satisfied (defends against a race between validation and commit).
--
-- Only the two product loops change; every other check, the appointment
-- capacity/business-hours validation, and the vehicle-handling logic are
-- byte-for-byte unchanged from 202607270001_align_order_totals_with_product_payments.sql.
