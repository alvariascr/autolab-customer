...
begin
  ...
  if (
    select count(
      distinct coalesce(input.inventory_item_id, input."inventoryItemId")
    )
  ...
  for v_product in
    ...
    from (
      select
        coalesce(inventory_item_id, "inventoryItemId") as inventory_item_id,
      ...
      group by coalesce(inventory_item_id, "inventoryItemId")
    ) input
  ...
  loop
    ...
    -- inventory_items.selling_price is the final customer price, tax included.
    v_products_total :=
  ...
  -- Keep order totals aligned with the final amount the customer sees.
  -- Tax is included in product prices and must not be added again.
  v_total_amount := v_products_total + v_delivery_fee;
  ...
  -- Business rule: creating the cart order reserves stock immediately. If
  -- Laropay is not completed, the order remains payable directly at workshop.
  for v_product in
    select
      id,
      selling_price,
