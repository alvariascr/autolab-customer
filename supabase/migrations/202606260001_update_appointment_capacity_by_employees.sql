CREATE OR REPLACE FUNCTION public.get_workshop_active_employee_count(
  p_workshop_id uuid
)
RETURNS integer
LANGUAGE sql
...
AS $$
  ...
  where e.workshop_id = p_workshop_id
    ...
    and w.status in ('active', 'complete');
$$;

REVOKE ALL ON FUNCTION public.get_workshop_active_employee_count(uuid)
FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.get_workshop_active_employee_count(uuid)
TO authenticated;

DROP FUNCTION IF EXISTS public.book_service_appointment(
  uuid,
  uuid,
  date,
